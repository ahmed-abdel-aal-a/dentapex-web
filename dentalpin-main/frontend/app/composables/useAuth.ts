...
export function useAuth() {
  ...
  function syncTokensToStorage(access: string | null, refresh: string | null) {
    ...
    try {
      if (access) {
        localStorage.setItem('dentapex_access_token', access)
      } else {
        localStorage.removeItem('dentapex_access_token')
      }
      if (refresh) {
        localStorage.setItem('dentapex_refresh_token', refresh)
      } else {
        localStorage.removeItem('dentapex_refresh_token')
      }
    } catch {
  ...
  async function init(): Promise<void> {
    ...
    try {
      ...
      if (import.meta.client && typeof window !== 'undefined' && window.localStorage) {
        try {
          if (!accessToken.value) {
            const storedAccess = localStorage.getItem('dentapex_access_token')
            if (storedAccess) accessToken.value = storedAccess
          }
          if (!refreshToken.value) {
            const storedRefresh = localStorage.getItem('dentapex_refresh_token')
